...
class HerbMedicine:
    ...
    def __init__(self, name=None, source=None, indication=None, effect=None, note=None, composition=None, factorization=None):
        ...
        self.composition = composition if composition is not None else self.save_list(composition)
...
class HerDataBase:
    ...
    def __init__(self, address, factorization_sheet=None, composition_sheet=None, basic_sheet=None, herb_sheet=None):
        # 인수분해 결과 시트, 처방구성 시트, 기본방 시트, 약재 시트
        self.address = address
        self.factorization_sheet = factorization_sheet
        self.composition_sheet = pd.read_excel(self.address, sheet_name="처방")
...
def list_check(composition_list):
...
address_DB = HerDataBase(address=address_DB_input)
if address_DB.addrest_check():
    pass
else:
...
print("________________________________________________\n")
  ...
  # 1-1. '처방명'만 입력하면 먼저 기본방 시트에서, 다음으로는 처방구성 시트에서 해당 처방명을 검색해 구성을 불러옵니다.
...
if STATUS != "input_nulNnul":
    if list_check(composition_list):
        STATUS = "input_list_OK"
    else:
...
# SHEET 단계
# 2.  입력된 '구성'에 '기본방' 시트의 '구성'이 포함되어 있는지 검색합니다. *(미리 제작된 class를 이용해 '처방명' 또는 '구성'을 이름으로 가지는 객체를 생성합니다.)*

  # 2-1. 먼저 '기본방' 시트의 '구성' 중 전체가 그대로 포함되어 있는지 검색합니다.
    # 2-1-1. 만약 그대로 포함되어 있다면 기본방 '처방명'과, 그의 '주치'와 '효능'을 불러와서 함께 저장합니다.
  # 2-2. 그대로 포함되어 있지 않다면 '기본방' 시트의 '구성' 중 '주약'이 모두 포함되어 있는 '처방명'이 있는지 검색합니다.
    # 2-2-1. '주약'이 모두 포함되어 있는 '처방명'이 있음이 확인되면, 해당 '처방명'의 다른 '구성' 요소 중 입력된 '구성'에 포함된 것이 있는지 확인합니다. 있다면 기본방 '처방명'과, 그의 '주치'와 '효능', 그리고 포함된 '구성'을 불러와서 함께 저장합니다.
  # 2-3. 이 모든 과정은 중복으로 이뤄집니다. 포함된 기본방 '구성'이 있다 한들 제외하고 다음 검색을 실행하지 않습니다.
